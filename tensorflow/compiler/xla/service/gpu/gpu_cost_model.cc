...
double EstimateHloModuleCost(const HloModule* hlo_module) {
  ...
  ProfilingResult prof_result(
      pass_context::GetPyObject("hlo_cost_model::profiling_result"));
  ...
  for (const HloInstruction* ins : entry_computation->instructions()) {
    ...
    if (ins->opcode() == HloOpcode::kAllGather || 
      ...
      for (const auto operand : ins->operands()) {
        int64_t size = GetBytes(operand->shape());
        switch (ins->opcode()) {
          case HloOpcode::kAllGather:
            cost += prof_result.EstimateAllGatherCost(replica_groups, size,
                    operand->shape().element_type());
            break;
          case HloOpcode::kAllReduce:
            cost += prof_result.EstimateAllReduceCost(replica_groups, size,
                    operand->shape().element_type());
            break;
          case HloOpcode::kAllToAll:
            cost += prof_result.EstimateAllToAllCost(replica_groups, size,
                    operand->shape().element_type());
            break;
          case HloOpcode::kReduceScatter:
            cost += prof_result.EstimateReduceScatterCost(replica_groups, size,
                    operand->shape().element_type());
            break;
          default:
            break;
    ...
    if (ins->IsCustomCall(kGemmCallTarget)) {
      double flop_count = 0;
      cost += prof_result.EstimateDotCost(flop_count, ins->shape().element_type());
    }
