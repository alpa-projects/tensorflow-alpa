...
public: 
  ...
  void addEdge(Item *from, Item *to) {
    if (from->isSwap || to->isSwap) {
      ...
      VLOG(3) 
        << "\tadd edge from: " << from->instruction->ToString() 
...
class MemoryRecorder {
 ...
 private: 
  struct Buffer {
    ...
    bool isSwappedIn() {
      return latest_alloc != defining_instruction;
    }

    // set if the buffer is in GPU
    void setInGPU(Item *swap_in_inst){
      // check: defining_instruction == nullptr << "already in with define instruction " << defining_instruction->ToString() << " while meeting another define instruction " << swap_in_inst->ToString();
      latest_alloc = swap_in_inst;
    }
  };

  // release with some policy, return the item defines the released buffer
  std::vector<std::pair<BufferId, Item*>> canAllocWithRelease(int64 size, Item *item) const {
    // todo: not only FIFO
    std::vector<std::pair<BufferId, Item*>> result;
    for (auto &p : alloced) {
      // cannot release a will used buffer
      if (absl::c_linear_search(item->buffers_used, p.first))
        continue;
  ...
  // register a swap out to CPU/release after exec: record the size and instruction
  ...
  void getSpaceFor(int64 size, Item *item) {
    if (!allocFreeMemory(size)) {
      ...
      if (released.first) {
        ...
        VLOG(3) << "\tswap out when getting space for instruction: " << item->instruction->ToString();
        ...
        for (auto& toRelease : toReleasePairs) {
          ...
          if (swap_out_inst.at(toReleaseBid) != nullptr) {
            ...
            registerRelease(lastUseInst, toReleaseBid, rest_size > 0 ? 0 : -rest_size);
  ...
  Buffer& CreateBufferFromLogicalBuffer(
    ...
    UsesList users = GetUsers(instruction_list, logical_buffer, points_to_analysis, &has_indirect_uses); 
    return NewBuffer(instruction_list.GetItem(logical_buffer->instruction()), 
      logical_buffer->shape(), 
      logical_buffer->index(), 
  ...
  Buffer &NewBuffer(Item *defining_instruction, const Shape &shape, 
    ...
    buffers_.push_back(
      Buffer{
        buffer_id, defining_instruction, defining_instruction, 
        shape, size_function_(shape), live_out, has_indirect_uses, 
        index, uses, get_num_of_unique_users(uses), 
  ...
  void SelfCHECK() {
    ...
    for (auto iter = released_after.begin(); iter != released_after.end();++iter) {
    ...
    VLOG(1) << "\tfree: " << free_memory 
      << ", released: " << released_memory 
      << ", alloced: " << alloced_memory;
    if (memory_bound_ == -1) return;
    CHECK(alloced_memory + released_memory + free_memory == memory_bound_) << "memory leak. allocated are: " + allocated
    << "released are: ";
  }
...
MemoryRecorder::MemoryRecorder(
  ...
  : computation_(computation), instruction_list(inst_list), memory_bound_(memory_bound), size_function_(size_function) {
  ...
  for (auto *item = inst_list.first(); item != nullptr; item = inst_list.next(item)) {
    const HloInstruction * const instruction = item->instruction;

    for (const LogicalBuffer *logical_buffer : points_to_analysis.GetBuffersDefinedByInstruction(instruction)) {
      Buffer *buffer;
      ...
      //   }
        ...
        buffer = &CreateBufferFromLogicalBuffer(logical_buffer, points_to_analysis, ContainsKey(live_out_set, logical_buffer));
    ...
    for (const LogicalBuffer *logical_buffer : points_to_analysis.GetPointsToSet(instruction).CreateFlattenedSet()) {
...
std::pair<bool, std::vector<Item*>> MemoryRecorder::allocReleasedMemory(int64 size) {
  // todo: faster
  std::vector<Item*> result;
  Item* tail = nullptr;
  for (auto& p : released_after) {
  ...
  if (size > 0) 
    return std::make_pair(false, std::vector<Item *>());
  else {
    for (auto iter = released_after.begin(); iter != released_after.end();) {
      if (iter->first == tail) break;
...
Status MemoryRecorder::PrepareForInstruction(Item *item) {
  ...
  for (auto bid : item->buffers_used) {
    ...
    if (buffer.inGPU()) { 
      if (buffer.isSwappedIn())
        buffer.defining_instruction
          ->instruction
          ->ReplaceUseWith(item->instruction, buffer.latest_alloc->instruction);
    ...
    buffer.defining_instruction
      ->instruction
      ->ReplaceUseWith(item->instruction, swapIn->instruction);
    last_use_inst.at(bid) = item;
  }
  SelfCHECK();
...
StatusOr<bool> HloSwapInsertion::SwapInsertionComputation(HloComputation* computation,
  ...
  for (auto* item = instruction_list.first(); item != nullptr;
    ...
    // if (callsite != nullptr && callsite->context() == CallContext::kSequential) { 
