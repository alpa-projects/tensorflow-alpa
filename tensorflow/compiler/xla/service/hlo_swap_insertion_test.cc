...
class HloSwapInsertionTest : public HloTestBase {
 protected:
  ...
  /*
  This computation is more close to a good test environment, as MLP has too many parameters and too few variables
  without swap: 
  // a, b, c alive
  d = a + b // a, b, c, d alive
  e = c + d // a, c, d, e alive
  ...
  */
  ...
  In this test, if the space is sizeof(F32) * (421(parameter)+20), x should be offloaded to make space for `lp_1 = binary(add, b_1, matmul_1)'
  ...
  */
  std::unique_ptr<HloComputation> MakeMLPComputation(
    ...
    auto matmul_1 = builder.AddInstruction(
      HloInstruction::CreateDot(vec20_shape_, param_1, x, dot_dnums, DefaultPrecisionConfig(2))
    );
    auto lp_1 = builder.AddInstruction(
      HloInstruction::CreateBinary(vec20_shape_, HloOpcode::kAdd, matmul_1, param_3)
    );
    auto matmul_2 = builder.AddInstruction(
      HloInstruction::CreateDot(vec10_shape_, param_2, lp_1, dot_dnums, DefaultPrecisionConfig(2))
    );
    // auto lp_2 = builder.AddInstruction(
    //   HloInstruction::CreateBinary(vec10_shape_, HloOpCode::kAdd, matmul_2, param_4)
    // );
    builder.AddInstruction(
      HloInstruction::CreateBinary(vec10_shape_, HloOpcode::kAdd, matmul_2, x)
    );
    return builder.Build();
  }
